...
const allowedMediaTypes = new Set(['welcome', 'announcement', 'hadith', 'text', 'kasTable', 'kastable', 'infoTable', 'infotable', 'donation', 'image', 'video', 'youtube', 'livestream']);
...
let hadisCache: { loadedAt: number; item: any | null } = { loadedAt: 0, item: null };
let doaCache: { loadedAt: number; item: any | null } = { loadedAt: 0, item: null };
...
function buildDisplay(data: Rows): Record<string, unknown> {
  ...
  return {
    ...
    mediaItems: data.media_items.filter((item) => allowedMediaTypes.has(item.type) && activeNow(item)).sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0)).slice(0, 10),
  };
}
...
async function randomDoaSlide(): Promise<any | null> {
  const now = Date.now();
  if (now - doaCache.loadedAt > 1_800_000 || !doaCache.item) {
    try {
      const response = await fetch('https://equran.id/api/doa');
      if (!response.ok) throw new Error(`eQuran Doa API HTTP ${response.status}`);
      const result = await response.json();
      const items = (Array.isArray(result?.data) ? result.data : []).filter((item: any) => /[\u0600-\u06FF]/.test(item?.ar || ''));
      if (items.length) doaCache = { loadedAt: now, item: items[Math.floor(Math.random() * items.length)] };
    } catch (_) {}
  }
  const item = doaCache.item;
  if (!item?.ar) return null;
  return {
    id: 'random_doa',
    title: 'DOA PILIHAN',
    type: 'hadith',
    content: JSON.stringify({ subtitle: item.nama || item.grup || '', arabic_text: item.ar, content: item.idn || item.tr || '' }),
    duration_seconds: 10,
    sort_order: -10000,
    is_active: true,
  };
}
async function randomHadisSlide(): Promise<any | null> {
  const now = Date.now();
  if (now - hadisCache.loadedAt > 1_800_000 || !hadisCache.item) {
    try {
      const response = await fetch('https://api.myquran.com/v3/hadis/enc/random');
      if (!response.ok) throw new Error(`MyQuran Hadis API HTTP ${response.status}`);
      const result = await response.json();
      const data = result?.data, text = data?.text;
      if (text?.ar && text?.id) hadisCache = { loadedAt: now, item: data };
    } catch (_) {}
  }
  const item = hadisCache.item;
  if (!item?.text?.ar || !item?.text?.id) return null;
  return {
    id: 'random_hadis',
    title: 'HADIS PILIHAN',
    type: 'hadith',
    content: JSON.stringify({
      subtitle: item.grade ? `Grade: ${item.grade}` : '',
      arabic_text: item.text.ar,
      content: [item.text.id, item.takhrij ? `Takhrij: ${item.takhrij}` : ''].filter(Boolean).join('\n'),
    }),
    duration_seconds: 10,
    sort_order: -9999,
    is_active: true,
...
async function adminRoute(req: Request, route: string, client: AdminClient, baseUrl: string, anonKey: string): Promise<Response> {
  ...
  const admin = await requireAdmin(req, baseUrl, client);
  const authToken = req.headers.get('authorization')!;
  if (route === '/api/v1/admin/account' && method === 'GET') {
    const response = await fetch(`${baseUrl}/api/auth/profiles/${encodeURIComponent(admin.id)}`);
    const result = response.ok ? await response.json() : {};
    return ok({ email: admin.email, profile: result.profile ?? result ?? {} });
  }
  if (route === '/api/v1/admin/account/profile' && method === 'PUT') {
    const b = await readJson(req);
    const name = text(b.name, 'Nama', 100, true);
    const avatarUrl = text(b.avatar_url, 'URL foto profil', 500);
    if (avatarUrl && !/^https?:\/\//i.test(avatarUrl)) throw new Error('URL foto profil tidak valid');
    const response = await fetch(`${baseUrl}/api/auth/profiles/current`, { method: 'PATCH', headers: { Authorization: authToken, 'Content-Type': 'application/json' }, body: JSON.stringify({ profile: { name, avatar_url: avatarUrl || null } }) });
    if (!response.ok) throw new Error('Profil akun gagal diperbarui');
    return ok(await response.json(), 'Profil akun berhasil diperbarui');
  }
  if (route === '/api/v1/admin/account/password/code' && method === 'POST') {
    const response = await fetch(`${baseUrl}/api/auth/email/send-reset-password`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: admin.email }) });
    if (!response.ok) throw new Error('Kode ganti password gagal dikirim');
    return ok({}, 'Kode verifikasi dikirim ke email admin');
  }
  if (route === '/api/v1/admin/account/password' && method === 'PUT') {
    const b = await readJson(req);
    const code = text(b.code, 'Kode verifikasi', 6, true);
    const password = text(b.password, 'Password baru', 200, true);
    if (!/^\d{6}$/.test(code)) throw new Error('Kode verifikasi harus 6 digit');
    if (password.length < 6) throw new Error('Password baru minimal 6 karakter');
    if (password !== b.password_confirmation) throw new Error('Konfirmasi password tidak sama');
    const exchange = await fetch(`${baseUrl}/api/auth/email/exchange-reset-password-token`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: admin.email, code }) });
    if (!exchange.ok) return fail('Kode verifikasi tidak valid atau kedaluwarsa', 400);
    const { token } = await exchange.json();
    const reset = await fetch(`${baseUrl}/api/auth/email/reset-password`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ newPassword: password, otp: token }) });
    if (!reset.ok) throw new Error('Password gagal diperbarui');
    return ok({}, 'Password berhasil diperbarui. Silakan login kembali');
  }
  ...
  if (route === '/api/v1/admin/dashboard/summary' && method === 'GET') return ok({ displayStatus: 'ONLINE', activeLocation: display.prayerLocation, lastSync: data.prayer_schedules.at(-1)?.updated_at ?? null, syncStatus: 'TERBARU (MyQuran API)', activeMediaCount: data.media_items.filter((x) => allowedMediaTypes.has(x.type) && x.is_active).length, activeRunningTextCount: data.running_texts.filter((x) => x.is_active).length, mosqueProfile: display.mosqueProfile });
  ...
  if (route === '/api/v1/admin/syuruq-settings' && method === 'GET') return ok(display.syuruqSettings);
  if (route === '/api/v1/admin/syuruq-settings' && method === 'PUT') {
    const b = await readJson(req);
    const payload = {
      syuruq_enabled: Boolean(b.is_enabled),
      syuruq_duration_minutes: int(b.durationMinutes, 1, 60),
      syuruq_display_message: text(b.displayMessage, 'Pesan syuruq', 2000, true),
    };
    await updateSingleton(client, 'display_settings', payload);
    return ok({ is_enabled: payload.syuruq_enabled, durationMinutes: payload.syuruq_duration_minutes, displayMessage: payload.syuruq_display_message }, 'Pengaturan layar pengingat Syuruq berhasil diperbarui');
  }
  ...
  if (collection && route === collection.base && method === 'GET') return ok(data[collection.table].filter((x) => collection.table !== 'media_items' || allowedMediaTypes.has(x.type)).sort((a, b) => Number(a.sort_order ?? a.id) - Number(b.sort_order ?? b.id)).slice(0, collection.table === 'media_items' ? 10 : undefined).map((x) => collection.table === 'media_items' ? { ...x, url: x.file_path } : x));
  if (collection && route === collection.base && method === 'POST') {
    ...
    if (collection.table === 'running_texts') payload = { text: text(b.text, 'Isi running text', 4000, true), speed: int(b.speed, 10, 200, 50), is_active: b.is_active !== false, starts_at: validDate(b.starts_at), ends_at: validDate(b.ends_at) };
      ...
      const type = text(b.type || 'announcement', 'Tipe media', 50, true);
      if (!allowedMediaTypes.has(type)) throw new Error('Tipe media tidak didukung');
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if ((route === '/api/v1/display' || route === '/api/v1/display/state' || route === '/') && req.method === 'GET') {
          const display = buildDisplay(await allRows(client)) as any;
          const [doa, hadis] = await Promise.all([randomDoaSlide(), randomHadisSlide()]);
          display.mediaItems = [doa, hadis, ...(display.mediaItems || [])].filter(Boolean);
          return ok(display);
        }
